...
$sql_columns = " SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name = '$table';";
$res = pg_query($conn, $sql_columns);
while($row = pg_fetch_row($res)) {
    $structures[] = $row[0];
}

$sql_data = " SELECT * FROM daya_tampung WHERE last_update > last_sync";
